...
def fetch_general_news():
    """
    Cached fetch of general market news to avoid redundant API calls.
    """
...
if st.button("Analyze News"):
    ...
    for article in articles:
        ...
        url = article.get("url")
        already = is_processed(url) if url else False
        if url and not already:
            log_sentiment("market", sentiment, log_date=pub_date.date())
            mark_processed(url, process_date=pub_date.date())
